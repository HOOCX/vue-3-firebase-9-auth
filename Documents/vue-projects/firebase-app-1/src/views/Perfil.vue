...
const onFinish = async () =>{
    ...
    if(fileList.value[0]) {
      ...
      const error = await userStore.updateImg(image);
      if (error) {
        return message.error('Hubo un error al subir la imagen');
        
      }
      return message.success('Se actualizó tu imagen con éxito');
    }
